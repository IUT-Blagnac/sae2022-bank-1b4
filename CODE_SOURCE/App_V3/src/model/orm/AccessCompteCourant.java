...
public class AccessCompteCourant {
	...
	public void insertCompte(CompteCourant compte)
		...
		try {
			...
			Connection connex = LogToDatabase.getConnexion();
			...
			System.err.println(query);
			PreparedStatement pst2 = connex.prepareStatement(query);
    ...
    public void creerCompte(CompteCourant compte)
        ...
        try {
            ...
            Connection con = LogToDatabase.getConnexion(); // Connexion à la BD

            compte.debitAutorise = -compte.debitAutorise;

            CallableStatement call; // crée une préparation de la procédure

            String q = "{call CreerCompte (?, ?, ?, ?)}"; // Contenu de la procédure tjr en prep
            call = con.prepareCall(q); // prépare la procédure sur la BD
            call.setInt(1, compte.debitAutorise); // change valeur 1er point ?
            call.setDouble(2, compte.solde); // change valeur 2eme point ?
            call.setInt(3, compte.idNumCli); // change valeur 3eme point ?
            call.registerOutParameter(4, java.sql.Types.INTEGER); // Ce param attend un entier (solde)

            call.execute();

            int res = call.getInt(4); // prend le 4eme param (solde)

            if (res == -1) // si moins -1 = erreur
